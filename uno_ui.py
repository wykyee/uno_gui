...
class GameFrame(Frame):
    ...
    def draw_void(self):
        Label(width=10, height=7).place(
            x=len(self.game.players[0].hand)*80+108, y=120
        )
    ...
    def draw_hand_people(self):
        ...
        for i in range(len(player.hand)):
            Label(self, text=f'{i+1}', font='Arial 14').place(
                x=108 + i * 80, y=100
            )
            ...
            btn.append(Button(image=render,
                              command=lambda c=i: self.on_click(c),
                              bd=0))
            btn[i].image = render
